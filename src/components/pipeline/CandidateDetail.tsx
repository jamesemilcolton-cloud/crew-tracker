...
export function CandidateDetail({ candidate, onClose, onUpdate, onArchive }: CandidateDetailProps) {
  ...
  return (
    <div className="glass-panel-elevated h-full flex flex-col max-h-[65vh] relative">
      {/* Sticky header with exit and delete buttons - well separated */}
      <div className="sticky top-0 z-10 bg-card/95 backdrop-blur-sm border-b border-border/30 px-6 py-3 flex items-center justify-between shrink-0 rounded-t-xl">
        <button onClick={onClose} className="text-muted-foreground hover:text-foreground transition-colors flex items-center gap-1.5 text-sm">
          <X className="w-5 h-5" />
          <span>Close</span>
        </button>
